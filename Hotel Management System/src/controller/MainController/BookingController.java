...
public class BookingController {
    ...
    public boolean addBookingDetails(BookingDetails bd, String id) throws SQLException, ClassNotFoundException {
      ...
      Connection con = null;
        try {
            ...
            if (pst.executeUpdate() > 0) {
                if (new BookingController().deleteAvailableRoom(id)) {
                    con.commit();
                    return true;
                } else {
                    con.rollback();
    ...
    public static boolean deleteAvailableRoom(String id) throws SQLException, ClassNotFoundException {
        Connection con = DbConnection.getInstance().getConnection();
        PreparedStatement pst = con.prepareStatement("DELETE FROM AvailableRoom WHERE roomId=?");
        pst.setObject(1,id);
        return pst.executeUpdate()>0;
    }
